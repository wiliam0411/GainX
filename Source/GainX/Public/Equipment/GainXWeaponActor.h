...
#include "GainXWeaponActor.generated.h"

/**
 *
 */
UCLASS()
class GAINX_API AGainXWeaponActor : public AGainXEquipmentActor
{
    GENERATED_BODY()
...
public:
    ...
    void FireFX(const TArray<FHitResult>& Hits);

    UFUNCTION(BlueprintCallable, Category = "GainX|WeaponInstance")
    int32 GetBulletsPerCartridge() const { return BulletsPerCartridge; }

    UFUNCTION(BlueprintCallable, BlueprintNativeEvent, Category = "GainX|WeaponInstance")
    ...
    UFUNCTION(BlueprintCallable, Category = "GainX|WeaponInstance")
    float GetFireInterval() const { return FireRate != 0.0f ? 60.0f / FireRate : 1.0f; }
...
protected:
    ...
    // Overrideable sight transform variable
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Weapon Instance|Weapon Config")
    FTransform SightTransform = FTransform::Identity;

    // Distance from the sight to the camera
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Weapon Instance|Weapon Config")
    float SightDistance = 0.0f;

    // Duration of transition from hipfire to ADS
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Weapon Instance|Weapon Config")
    float ADSActivationTime = 0.0f;

    // Duration of transition from ADS to hipfire
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Weapon Instance|Weapon Config")
    float ADSDeactivationTime = 0.0f;
