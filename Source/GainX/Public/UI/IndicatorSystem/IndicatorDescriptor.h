...
class UGainXIndicatorManagerComponent;
class UUserWidget;
struct FFrame;
struct FSceneViewProjectionData;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnIndicatorWidgetUpdateDelegate, float, IndicatorProgress);
...
class GAINX_API UIndicatorDescriptor : public UObject
{
    GENERATED_BODY()
...
public:
    ...
    FOnIndicatorWidgetUpdateDelegate OnIndicatorWidgetUpdate;

public:
    UFUNCTION(BlueprintCallable)
    void SetAutoRemoveWhenIndicatorComponentIsNull(bool CanAutomaticallyRemove) { bAutoRemoveWhenIndicatorComponentIsNull = CanAutomaticallyRemove; }
    UFUNCTION(BlueprintCallable)
    bool GetAutoRemoveWhenIndicatorComponentIsNull() const { return bAutoRemoveWhenIndicatorComponentIsNull; }

    bool CanAutomaticallyRemove() const { return bAutoRemoveWhenIndicatorComponentIsNull && !IsValid(GetSceneComponent()); }
...
public:
    ...
    UFUNCTION(BlueprintCallable)
    void SetDesiredVisibility(bool InVisible) { bVisible = InVisible; }

    UFUNCTION(BlueprintCallable)
    EActorCanvasProjectionMode GetProjectionMode() const { return ProjectionMode; }
    UFUNCTION(BlueprintCallable)
    void SetProjectionMode(EActorCanvasProjectionMode InProjectionMode) { ProjectionMode = InProjectionMode; }
...
public:
    UGainXIndicatorManagerComponent* GetIndicatorManagerComponent() { return ManagerPtr.Get(); }
    void SetIndicatorManagerComponent(UGainXIndicatorManagerComponent* InManager);
...
private:
    ...
    UPROPERTY()
    TWeakObjectPtr<UGainXIndicatorManagerComponent> ManagerPtr;
