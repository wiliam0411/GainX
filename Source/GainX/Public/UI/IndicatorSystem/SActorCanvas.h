...
public:
    ...
    public:
        FSlot(UIndicatorDescriptor* InIndicator)
            : TSlotBase<FSlot>(), Indicator(InIndicator), ScreenPosition(FVector2D::ZeroVector), Depth(0), Priority(0.f), bIsIndicatorVisible(true), bInFrontOfCamera(true), bHasValidScreenPosition(false), bDirty(true), bWasIndicatorClamped(false), bWasIndicatorClampedStatusChanged(false)
        {
        }
    ...
    SActorCanvas() : CanvasChildren(this), ArrowChildren(this), AllChildren(this)
    {
...
private:
    ...
    void GetOffsetAndSize(const UIndicatorDescriptor* Indicator, FVector2D& OutSize, FVector2D& OutOffset, FVector2D& OutPaddingMin, FVector2D& OutPaddingMax) const;
...
private:
    ...
    TWeakObjectPtr<UGainXIndicatorManagerComponent> IndicatorComponentPtr;
